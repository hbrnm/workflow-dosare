-- Migrare 36: self-heal admin la salvarea setărilor atelierului
-- Rulează DUPĂ migrarea 35 (sau înlocuiește RPC-ul dacă 35 lipsește).
--
-- Problema: utilizatorul e admin în UI / setari, dar atelier_membri.role != 'admin'
-- → update_atelier_settings / RLS blochează redenumirea.
--
-- Ce face:
--   * promovează din setari.admin_emails + utilizatori role=admin pe toate membership-urile
--   * RPC update_atelier_settings: auto-promote sole member / admin_emails, apoi update

begin;

-- Heal broad: admin_emails
...
-- Heal: utilizatori JSON cu role admin
update public.atelier_membri m
set role = 'admin'
from public.setari s,
...
begin
  ...
  -- Self-heal: listat ca admin în setari
