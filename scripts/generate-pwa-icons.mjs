...
// 1. Write public/logo.svg (Horizontal Logo with Icon + Typography)
...
// 2. Write public/icon.svg (App Square Icon 512x512 vector)
...
// 3. Write public/maskable.svg (Adaptive Maskable Icon with full background & safe margin)
const maskableSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="512" height="512">
  ...
  <!-- Safe Zone Visual Ring (translucent) -->
  <circle cx="256" cy="256" r="200" fill="rgba(255, 255, 255, 0.08)"/>

  <!-- Centered Logo Mark within 80% Safe Area -->
...
// pwa-192x192.png
const pwa192 = await sharp(iconBuffer)
  .resize(192, 192)
  .png({ compressionLevel: 9 })
  .toBuffer();
writeFileSync(join(root, "public/pwa-192x192.png"), pwa192);
console.log("Wrote public/pwa-192x192.png");

// pwa-512x512.png
const pwa512 = await sharp(iconBuffer)
  .resize(512, 512)
  .png({ compressionLevel: 9 })
  .toBuffer();
writeFileSync(join(root, "public/pwa-512x512.png"), pwa512);
console.log("Wrote public/pwa-512x512.png");

// Legacy icon-192.png & icon-512.png for backwards compatibility
writeFileSync(join(root, "public/icon-192.png"), pwa192);
writeFileSync(join(root, "public/icon-512.png"), pwa512);
...
// maskable-icon-512x512.png (512x512 full background)
const maskable512 = await sharp(maskableBuffer)
  .resize(512, 512)
  .png({ compressionLevel: 9 })
  .toBuffer();
writeFileSync(join(root, "public/maskable-icon-512x512.png"), maskable512);
console.log("Wrote public/maskable-icon-512x512.png");

// favicon.ico (32x32 PNG buffer written as favicon.ico)
