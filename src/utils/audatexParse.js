 ...
 * into claim financial fields + operațiuni (piese / manoperă).
 *
...
function mergeLineItemsToOperations(parts, labour, paint) {
  ...
  const upsert = (name, flags) => {
    ...
    byKey.set(key || `${byKey.size}_${clean}`, {
      id: uid(),
      piesa: clean.toUpperCase(),
      inl: !!flags.inl,
      rev: !!flags.rev,
      rep: !!flags.rep,
      uni: !!flags.uni,
...
function extractXmlOperations(xmlText) {
  ...
  while ((m = partRe.exec(xmlText))) {
    ...
    if (desc.length >= 3) {
      ops.push({ id: uid(), piesa: desc.toUpperCase(), inl: true, rev: false, rep: false, uni: false });
    }
  }
  const labourRe =
  ...
  while ((m = labourRe.exec(xmlText))) {
    ...
    if (desc.length >= 3) {
      ...
      ops.push({ id: uid(), piesa: desc.toUpperCase(), ...flags });
    }
  }
  return ops;
}
