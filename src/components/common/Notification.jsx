...
export default function Notification({ notice, onClose, stacked = false }) {
  ...
  const baseClasses = `flex items-center justify-between gap-3 px-4 py-3 rounded-xl shadow-2xl text-[13px] font-bold text-white ${isErr ? "bg-[#B23A2E]" : "bg-[#3E6B45]"}`;

  if (stacked) {
    return (
      <div className={`${baseClasses} w-80`}>
