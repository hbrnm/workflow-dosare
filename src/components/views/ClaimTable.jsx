  ...
  const renderRow = (c, i, { inGroup = false } = {}) => {
    ...
    return (
      ...
      >
        ...
        <td className="px-3 py-2 whitespace-nowrap text-[var(--app-muted)]">{c.asigurator || "—"}</td>
        ...
        <td className="px-3 py-2 whitespace-nowrap text-[var(--app-muted)]">{c.marcaModel || "—"}</td>
        <td className="px-3 py-2 whitespace-nowrap">
          ...
          {c.status === "piese_comandate" && c.dataComandaPiese && (
            <span className="app-table-parts-badge ml-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded">
              📦 {c.dataComandaPiese}
            </span>
          )}
          {overdue && <AlertBadge days={days} threshold={c.termenAlertaZile || 3} />}
        </td>
        <td className="px-3 py-2 whitespace-nowrap text-[var(--app-muted)]">{fmtDate(c.dataDeschiderii)}</td>
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="app-table-wrap overflow-x-auto rounded-lg">
        <table className="app-table w-full text-[12.5px]">
                  ...
                  <tr className="app-table-group-header">
                    <td colSpan={9} className="px-3 py-2">
                      <button
                        ...
                        className="flex items-center gap-2 text-[12px] font-extrabold w-full text-left"
                      >
                        <span className="font-mono uppercase">🚗 {first.numarInmatriculare}</span>
                        <span className="app-table-group-badge text-[10px] px-2 py-0.5 rounded-full">{group.length} dosare</span>
                        <span className="ml-auto flex items-center gap-1 text-[11px] text-[var(--app-muted)]">
