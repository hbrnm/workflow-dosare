...
export default function BriefZilnic({ claims, onOpen, onMoveToStatus, onDuplicate, canEditFn, pragRidicare, onSetPrag }) {
  ...
  return (
    <div className="space-y-3 flex flex-col flex-1 min-h-0">
      ...
      <div className="bg-white rounded-lg border border-[#DAD4C6] px-3 py-2 shadow-2xs flex flex-wrap items-center justify-between gap-2 shrink-0">
        <div className="flex items-center gap-2">
          <div className="text-[13.5px] font-bold text-[#23282E] capitalize" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
        ...
        <div className="flex items-center gap-2 text-[11.5px] bg-[#FAF8F5] px-2.5 py-1 rounded-lg border border-[#DAD4C6]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 shrink-0">
        <div className="bg-white border border-[#DAD4C6] rounded-lg px-2.5 py-1.5 flex items-center justify-between">
          <span className="text-[10.5px] text-[#6B6558] font-semibold">Total acțiuni:</span>
          <span className="text-[12.5px] font-bold font-mono text-[#23282E]">{totalActiuniAzi}</span>
        </div>
        <div className="bg-white border border-[#DAD4C6] rounded-lg px-2.5 py-1.5 flex items-center justify-between">
          <span className="text-[10.5px] text-[#6B6558] font-semibold">Programate azi:</span>
          <span className="text-[12.5px] font-bold font-mono text-[#3B5166]">{programariAzi.length}</span>
        </div>
        <div className="bg-white border border-[#DAD4C6] rounded-lg px-2.5 py-1.5 flex items-center justify-between">
          <span className="text-[10.5px] text-[#6B6558] font-semibold">Finalizate azi:</span>
          <span className="text-[12.5px] font-bold font-mono text-[#3E6B45]">{gataAzi.length}</span>
        </div>
        <div className={`bg-white border rounded-lg px-2.5 py-1.5 flex items-center justify-between ${neridicateVechi.length ? "border-[#B23A2E]" : "border-[#DAD4C6]"}`}>
          <span className="text-[10.5px] text-[#6B6558] font-semibold">De sunat:</span>
          <span className={`text-[12.5px] font-bold font-mono ${neridicateVechi.length ? "text-[#B23A2E]" : "text-[#3E6B45]"}`}>{neridicateVechi.length}</span>
        </div>
        <div className={`bg-white border rounded-lg px-2.5 py-1.5 flex items-center justify-between ${masiniSchimbDepasite.length ? "border-[#B23A2E]" : "border-[#DAD4C6]"}`}>
          <span className="text-[10.5px] text-[#6B6558] font-semibold">Auto schimb depășite:</span>
          <span className={`text-[12.5px] font-bold font-mono ${masiniSchimbDepasite.length ? "text-[#B23A2E]" : "text-[#3E6B45]"}`}>{masiniSchimbDepasite.length}</span>
        </div>
      </div>
